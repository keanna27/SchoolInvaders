...
pygame.display.set_caption("School Invaders")

player_img = pygame.image.load("rocket.jpg")
enemy_img1 = pygame.image.load("enemy1.jpg")
master_enemy = pygame.image.load("boss.jpg")
...
class Enemy:
    def __init__(self, x, y):
        self.image = pygame.transform.scale(enemy_img1, (60, 60))
        self.x = x
        self.y = y
        self.speed = 3
        self.direction = 1
...
class MasterEnemy:
    def __init__(self):
        self.image = pygame.transform.scale(master_enemy, (200, 200))
        self.x = WIDTH // 2 - 50
        self.y = 50
        self.speed = 2
        self.health = 500
...
class SchoolInvaders:
    ...
    def run(self):
        while self.running:
            screen.fill(WHITE)  # Changed background color to white
            self.handle_events()
            self.update()
            self.draw()
    ...
    def update(self):
        ...
        if self.enemy_count >= 30:
            ...
            for bullet in self.master_enemy.bullets:
                bullet.move()
                if self.player.x < bullet.x < self.player.x + 50 and bullet.y > self.player.y:
    ...
    def draw(self):
        ...
        score_text = font.render(f"Enemies Defeated: {self.enemy_count}", True, BLACK)
        screen.blit(score_text, (10, 10))

        health_text = font.render(f"Player HP: {self.player.health}", True, GREEN)
